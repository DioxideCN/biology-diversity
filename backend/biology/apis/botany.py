...
from biology.models import Botany
...
# 创建Botany
@router.post("/botany", response=BotanySchemaOut)
def create_botany(request, data: BotanySchemaIn):
    # 调用封装的create函数
    botany = create(request, data, Botany)
    return botany


# 删除Botany
@router.delete("/botany/{botany_id}")
def delete_botany(request, botany_id: int):
    delete(botany_id, Botany)
    return {"success": True}


# 更新Botany
@router.put("/botany/{botany_id}", response=BotanySchemaOut)
def update_botany(request, botany_id: int, data: BotanySchemaIn):
    botany = update(request, botany_id, data, Botany)
    return botany


# 获取Botany 并分页
@router.get("/botany", response=List[BotanySchemaOut])
@paginate(MyPagination)
def list_botany(request, filters: Filters = Query(...)):
    qs = retrieve(request, Botany, filters)
    return qs


# 获取所有植物
@router.get("/botany/all/list", response=List[BotanySchemaOut])
def all_list_botany(request):
    qs = retrieve(request, Botany)
    return qs


# 根据id获取单个植物
@router.get("/botany/{botany_id}", response=BotanySchemaOut)
def get_botany(request, botany_id: int):
    botany = get_object_or_404(Botany, id=botany_id)
    return botany
