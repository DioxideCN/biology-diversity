...
from biology.models import Botany_knowledge
...
class BotanySchemaIn(ModelSchema):
    class Config:
        model = Botany_knowledge
        model_fields = ['name', 'description', 'habitat', 'live', 'reproduction', 'distribution',
                        'subclass_distribution', 'family_distribution', 'protection_level', 'notes', 'botany_id']
...
# 创建Botany
@router.post("/botany_knowledge", response=BotanySchemaOut)
def create_botany(request, data: BotanySchemaIn):
    # 调用封装的create函数
    botany = create(request, data, Botany_knowledge)
    return botany


# 删除Botany
@router.delete("/botany_knowledge/{botany_id}")
def delete_botany(request, botany_id: int):
    delete(botany_id, Botany_knowledge)
    return {"success": True}


# 更新Botany
@router.put("/botany_knowledge/{botany_id}", response=BotanySchemaOut)
def update_botany(request, botany_id: int, data: BotanySchemaIn):
    botany = update(request, botany_id, data, Botany_knowledge)
    return botany


# 获取Botany 并分页
@router.get("/botany_knowledge", response=List[BotanySchemaOut])
@paginate(MyPagination)
def list_botany(request, filters: Filters = Query(...)):
    qs = retrieve(request, Botany_knowledge, filters)
    return qs


# 获取所有植物
@router.get("/botany_knowledge/all/list", response=List[BotanySchemaOut])
def all_list_botany(request):
    qs = retrieve(request, Botany_knowledge)
    return qs


# 根据id获取单个植物
@router.get("/botany_knowledge/{botany_id}", response=BotanySchemaOut)
def get_botany(request, botany_id: int):
    botany = get_object_or_404(Botany_knowledge, id=botany_id)
    return botany
